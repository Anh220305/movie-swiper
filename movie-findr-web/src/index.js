...
const fetchSwipeContents = (username) => {
    const response = await fetch('http://localhost:8000/backend/api/novel_movies/jomanw/',
...
// TODO make API request
const postSwipeResults = (cards, username) => {
    var ratings = {};
    for (var i = 0; i < cards.length; i++) {
        // TODO make this not always true (pass in whether or not they liked the move)
        ratings[cards[i].movieDbId] = true;
    }
    const novel_movies_response = fetch(
        'http://localhost:8000/backend/api/rate_movie',
        {method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({moviedb_id_to_rating:ratings, username:username})}
    )
...
const Swipe = (props) => {
    ...
    useEffect(() => {
        if (cardIndex >= cards.length) {
            postSwipeResults(cards, props.username);
            setCards(fetchSwipeContents(props.username));
            setCardIndex(0);
        }
    }, [cardIndex])
